...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log('Dialog Result: ', window.myAPI)
      } catch (err) {
    ...
    <div className="flex w-full h-full bg-white text-gray-900">
      ...
      <div className="w-60 min-w-[200px] border-r border-gray-200 bg-gray-50 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2">
          <div className="text-xs text-gray-500 uppercase tracking-wide px-2 py-1">My Planets</div>
          <div className="text-sm text-gray-400 px-2 py-4">No planets yet</div>
          <div className="text-xs text-gray-500 uppercase tracking-wide px-2 py-1 mt-4">
            Following
          </div>
          <div className="text-sm text-gray-400 px-2 py-4">Not following anyone</div>
        </div>
        {/* IPFS 状态指示器（占位） */}
      ...
      <div className="w-72 min-w-[240px] border-r border-gray-200 flex flex-col">
        <div className="p-3 border-b border-gray-200 text-sm font-medium">Articles</div>
